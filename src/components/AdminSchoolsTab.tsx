              ...
              <div>
                ...
                <div className="space-y-1.5 text-xs text-slate-600 dark:text-slate-300 mb-4 bg-slate-50 dark:bg-slate-950/60 p-3 rounded-xl border border-slate-100 dark:border-slate-800/80">
                  ...
                  <div className="flex items-center space-x-2 pt-1 flex-wrap gap-1">
                    ...
                    {(school.shifts && school.shifts.length > 0 ? school.shifts : ['MANHA', 'TARDE']).map((s) => (
                      <span
                        key={s}
                      ...
                      >
                        {s === 'MANHA' ? 'Manhã' : s === 'TARDE' ? 'Tarde' : s === 'NOITE' ? 'Noite' : 'Integral'}
                      </span>
                    ))}
                  </div>
                </div>
                ...
                <div className="sm:col-span-2">
                  <div className="flex items-center justify-between mb-1">
                    <label className="block text-[11px] font-bold text-slate-700 dark:text-slate-300">
                      Turnos de Atendimento (Restrição de Agendamento):
                    </label>
                    <span className="text-[10px] text-blue-600 dark:text-blue-400 font-bold">
                      {formData.shifts.length} {formData.shifts.length === 1 ? 'turno habilitado' : 'turnos habilitados'}
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mb-2">
                    Apenas os turnos selecionados abaixo ficarão disponíveis para reserva de salas e laboratórios nesta escola. Turnos desmarcados ficam estritamente bloqueados para agendamentos.
                  </p>
                      ...
                      return (
                        ...
                        >
                          <span className={`w-3.5 h-3.5 rounded-full flex items-center justify-center text-[9px] ${
                            isChecked ? 'bg-white text-blue-600' : 'border border-slate-400 text-transparent'
                          }`}>
                            ✓
                          </span>
                          <span>
