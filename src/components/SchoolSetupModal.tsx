...
export const SchoolSetupModal: React.FC<SchoolSetupModalProps> = ({
  ...
  const handleLogoFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2.5 * 1024 * 1024) {
      alert('A imagem é muito grande. Escolha um arquivo de imagem de até 2.5MB.');
      return;
    }
          ...
          <div className="space-y-3 p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between border-b border-slate-200/80 dark:border-slate-800 pb-1">
              <h4 className="text-xs font-bold text-slate-900 dark:text-slate-100 flex items-center gap-1.5 uppercase tracking-wide">
                ...
                3. Logo / Brasão Institucional da Escola (Opcional)
              </h4>
              <span className="text-[10px] text-slate-500">
                Personaliza cabeçalhos e comprovantes
              </span>
            </div>
            ...
            {logoUrl ? (
              <div className="flex flex-col sm:flex-row items-center gap-4 bg-white dark:bg-slate-900 p-3.5 rounded-xl border border-slate-200 dark:border-slate-800">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <div className="text-center">
                    ...
                    <span className="text-[9px] font-bold text-slate-500 uppercase mt-1 block">Claro</span>
                  </div>
                  <div className="text-center">
                    <div className="w-14 h-14 rounded-xl bg-slate-950 border border-slate-800 p-1.5 flex items-center justify-center shadow-xs">
                      <img
                        src={logoUrl}
                        alt="Pré-visualização do logo"
                        className="max-h-full max-w-full object-contain"
                    ...
                    <span className="text-[9px] font-bold text-slate-400 uppercase mt-1 block">Escuro</span>
                  </div>
                </div>
                ...
                <div className="flex-1 min-w-0 space-y-1.5 text-left w-full sm:w-auto">
                  <div className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-bold text-xs">
                    ...
                    <span>Brasão ativo para esta escola</span>
                  </div>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">
                    O logo aparecerá no cabeçalho do sistema, em impressões e no seletor de escolas da rede.
                  </p>
                  <div className="flex items-center gap-2 pt-1">
                    <label
                      htmlFor="modal-school-logo-input"
              ...
              <div className="space-y-3">
                ...
                <label
                  htmlFor="modal-school-logo-input"
                  className="border-2 border-dashed border-slate-300 dark:border-slate-700 hover:border-blue-500 dark:hover:border-blue-500 bg-white/50 dark:bg-slate-900/50 hover:bg-blue-50/50 dark:hover:bg-blue-950/20 rounded-2xl p-4 flex flex-col items-center justify-center gap-1 cursor-pointer transition-all text-center group"
                >
                  ...
                  <p className="text-xs font-bold text-slate-800 dark:text-slate-200">
                    Clique para selecionar um arquivo de logo ou brasão
                  </p>
                  <p className="text-[10px] text-slate-400">
                    PNG, JPG, SVG ou WebP (máx: 2.5MB)
                  </p>
                </label>
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    placeholder="https://exemplo.gov.br/brasao.png"
                    className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-slate-100 text-xs placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>

                {/* Pick institutional crest */}
                <div className="pt-2 border-t border-slate-200/80 dark:border-slate-800">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-2">
                    Ou escolha um brasão institucional:
                  </span>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {SAMPLE_SCHOOL_LOGOS.map((sample) => (
                      <button
                        key={sample.id}
                        type="button"
                        onClick={() => setLogoUrl(sample.dataUrl)}
                        className="p-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 hover:border-blue-500 dark:hover:border-blue-500 text-left transition-all flex items-center gap-2 cursor-pointer group"
                      >
                        <div className="w-7 h-7 rounded-lg bg-slate-50 dark:bg-slate-800 p-1 shrink-0 flex items-center justify-center border border-slate-100 dark:border-slate-700">
                          <img
                            src={sample.dataUrl}
                            alt={sample.name}
                        ...
                        <span className="text-[10px] font-bold text-slate-800 dark:text-slate-200 truncate group-hover:text-blue-500">
                          {sample.name}
                        </span>
                      </button>
                    ))}
